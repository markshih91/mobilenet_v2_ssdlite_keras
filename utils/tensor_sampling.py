...
def sample_tensors(weights_list, sampling_instructions, axes=None, init=None, mean=0.0, stddev=0.005):
    ...
    if (not isinstance(sampling_instructions, (list, tuple))) or (len(sampling_instructions) != first_tensor.ndim):
        raise ValueError(
            "The sampling instructions must be a list whose length is the number of \
            dimensions of the first tensor in `weights_list`.")
    ...
    for i, sampling_inst in enumerate(sampling_instructions):
        if isinstance(sampling_inst, (list, tuple)):
            ...
            if amax >= first_tensor.shape[i]:
                raise ValueError(
                    "The sample instructions for dimension {} contain index {}, \
                    which is greater than the length of that dimension.".format(
                        i, amax))
            ...
            raise ValueError(
                "Each element of the sampling instructions must be either an integer or \
                a list/tuple of integers, but received `{}`".format(
                    type(sampling_inst)))
    ...
    if up_sample:
        ...
        # Pick the indices of the elements in `upsampled_first_tensor`
        # that should be occupied by `subsampled_first_tensor`.
        up_sample_slices = [np.arange(k) for k in subsampled_first_tensor.shape]
        for i in up_sample:
            # Randomly select across which indices of this dimension to scatter
            # the elements of `new_weights_tensor` in this dimension.
